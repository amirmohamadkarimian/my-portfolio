...
const PROJECT_COLORS = ["#F5A623", "#F5A623", "#F5A623", "#F5A623", "#F5A623"];
...
function ProjectCard({ project, color, pattern, index, visible, isRTL }) {
  return (
    ...
    >
      <div
        className="project-card group bg-bg-card border border-border-subtle rounded-2xl overflow-hidden flex flex-col h-full"
        style={{
          "--card-glow": `${color}25`,
      ...
      >
        ...
        >
          <svg
            viewBox="0 0 100 100"
            className="w-full h-full opacity-60 transition-transform duration-500 ease-out group-hover:scale-105"
            style={{ color }}
        ...
        <div className={`p-6 flex flex-col flex-1 ${isRTL ? "text-right" : ""}`}>
          ...
          <div className="flex flex-wrap gap-2 mb-5">
            {project.tags.map((tag) => (
              <span
                key={tag}
                className="inline-flex items-center gap-1.5 font-mono text-xs text-text-secondary bg-bg-surface border border-border-subtle px-2.5 py-1 rounded-full hover:border-accent/40 transition-colors"
              >
                <TechIcon name={tag} className="w-3 h-3 flex-shrink-0" />
                {tag}
              </span>
            ))}
          </div>
